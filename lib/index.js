...
import { readFileSync, mkdirSync, writeFileSync, existsSync, readdirSync, statSync, renameSync, copyFileSync, utimesSync, rmSync } from 'node:fs'
...
export const Config = z.object({
   ...
   * `off` disables version checks entirely. */
  updateMode: z.enum(['manual', 'auto', 'off']).default('manual'),
...
/**
 * Wrap a raw ssh2 SFTP handle so a stuck server can never hang a tool call:
 * every method is bounded by the configured command timeout, and path args
 * are normalized via toSftpPath() so Windows remotes (drive-letter paths)
 * work too.
 */
function wrapSftp(client, sftp, config) {
  const withTimeout = (fn) => (...args) =>
    new Promise((r2, j2) => {
      const timer = setTimeout(() => j2(new Error('sftp operation timed out')), config.commandTimeoutMs)
      const done = (e, v) => {
        clearTimeout(timer)
        e ? j2(e) : r2(v)
      }
...
/**
 * Run one exec stream to completion: collects stdout/stderr (bounded), settles
 * on channel close or the configured command timeout. On timeout the remote
 * command gets SIGTERM and the channel is hard-closed so a runaway process
 * cannot keep holding the SSH connection.
 */
function runStream(client, stream, config, timeoutMs) {
  return new Promise((resolve, reject) => {
    ...
    let exitCode = null
    let exitSignal = null
    const hardCap = Math.max(config.maxOutputChars * 4, 1024 * 1024)
    const settle = () => {
      if (settled) return
      settled = true
      clearTimeout(timer)
      resolve({
        ...
        stderr: truncate(stderr, config.maxOutputChars),
      })
    }
      ...
      (client) =>
        new Promise((resolve, reject) => {
          client.exec(command, (err, stream) => {
            if (err) {
              // Same dead-connection recovery as sftp(): a channel open
              // failure means the pooled connection is stale — drop it and
              // retry ONCE on a fresh connection.
              if (/channel open failure|open failed/i.test(String((err && err.message) || err))) {
                this.invalidate()
                return this.connect().then(
                  (fresh) => fresh.exec(command, (err2, stream2) => {
                    if (err2) return reject(new Error('ssh exec failed: ' + ((err2 && err2.message) || err2)))
                    resolve(runStream(fresh, stream2, this.config, timeoutMs))
                  }),
            ...
            resolve(runStream(client, stream, this.config, timeoutMs))
          })
        }),
    )
  }
      ...
      (client) =>
        new Promise((resolve, reject) => {
          client.sftp((err, sftp) => {
            if (err) {
              // Channel open failure usually means the pooled connection is
              // dead server-side (idle timeout / network reset) while
              // keepalive hasn't noticed yet. Drop it and retry ONCE on a
              // fresh connection; a persistent error still rejects.
              if (/channel open failure|open failed/i.test(String((err && err.message) || err))) {
                this.invalidate()
                return this.connect().then(
                  (fresh) => fresh.sftp((err2, sftp2) => {
                    if (err2) return reject(new Error('ssh sftp failed: ' + ((err2 && err2.message) || err2)))
                    resolve(wrapSftp(fresh, sftp2, this.config))
                  }),
            ...
            resolve(wrapSftp(client, sftp, this.config))
          })
        }),
    )
  }
...
export async function apply(ctx, config) {
  ...
  // ── self-update (version check + apply) ────────────────────────────────────
  ...
  // files with the newer npm tarball. We never mutate node_modules directly
  // (pnpm owns it); instead we write the fresh files into the same directory
  // the loader already resolved, which is the profile's node_modules/dsh-remote
  // (hoisted) or the .pnpm store copy — both are writable for the local user.
  ...
  // A mode persisted via the settings UI (/dsh-remote/update-mode) wins over
  // the profile-config default, so the user's choice survives restarts without
  // editing cordis config.
  const persistedMode = (() => {
    try {
  ...
  if (persistedMode) config.updateMode = persistedMode
  /** Query the npm registry for the latest release; resolves null on any failure. */
  const fetchLatestVersion = async (timeoutMs = 8000) => {
    try {
   ...
   * package files (lib/*.js, cordis.patch.yml, package.json). A stale marker
   * file (.dsh-remote-updated) is written so the client can tell the user to
   * reload; the loader picks the new host half up on the next boot.
   */
  const applyUpdate = async (targetVersion) => {
    ...
    try {
      ...
      // Minimal tar.gz extractor: we only need package/{lib,package.json,cordis.patch.yml}.
      // Use node's zlib gunzip + a tiny tar parser for the regular files we ship.
  ...
  /** Minimal POSIX ustar parser (regular files only) for npm tarballs. */
  ...
  // Auto mode: check once on load and on an interval; apply silently when a
  // newer version exists. Failures are swallowed (never break the plugin).
  ...
  if (config.updateMode === 'auto') {
    ...
    void checkAndApply()
    updateTimer = setInterval(checkAndApply, Math.max(config.updateCheckIntervalMs, 60000))
    if (typeof updateTimer.unref === 'function') updateTimer.unref()
    ctx.effect(() => () => { if (updateTimer) clearInterval(updateTimer) }, 'dsh-remote.update-timer')
  }
  ...
  const routes = [
    ...
    {
      // Read a remote file over SFTP (live, not the local mirror) for the
      // sidebar's remote file viewer. Text files come back as UTF-8 content
      // (CRLF→LF), binary files are reported as binary with size+head bytes
      // (base64) so the client can sniff the format without loading the
      // whole file. `maxBytes` caps how much text is returned.
      kind: 'exact',
      ...
      handler: async (req, res) => {
        ...
        try {
          ...
          const target = normalizeRemotePath(p)
          const maxBytes = Math.min(Math.max(Number(q.get('maxBytes')) || 256 * 1024, 1024), 2 * 1024 * 1024)
          let sftp
          try {
            sftp = await pool.sftp()
          } catch (err) {
          ...
          // Binary sniff: NUL byte in the first 8KB (or the whole file when
          // smaller) marks the file binary — matches the sidebar's convention.
    ...
    },
    {
      ...
      handler: async (req, res) => {
        ...
        const latest = await fetchLatestVersion()
        if (latest === null) return sendJson(res, 200, { ok: false, current: currentVersion, error: '无法连接 npm registry' })
        const updateAvailable = gtVersion(latest, currentVersion)
        const updatedMarker = existsSync(path.join(selfDir(), '.dsh-remote-updated'))
    ...
    },
    {
      ...
      handler: async (req, res) => {
        ...
        try {
          ...
          // Persist to the plugin's install dir so it survives restarts; the
          // effective value is read back on next load by the host half.
          const dir = selfDir()
          try { writeFileSync(path.join(dir, 'update-mode'), mode) } catch (e) { return sendJson(res, 500, { ok: false, error: 'cannot persist mode: ' + ((e && e.message) || e) }) }
