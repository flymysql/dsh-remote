    ...
    },
    {
      ...
      handler: async (req, res) => {
        ...
        try {
          ...
          if (!dp || typeof dp.pick !== 'function') return sendJson(res, 400, { ok: false, error: '本地目录选择器不可用（本机需 DSH 原生 directory-picker backend）' })
          const picked = await Promise.resolve(dp.pick())
