...
//   • model tools `remote_info` / `remote_connect` / `remote_pick_workspace` /
//     `remote_list_dir` / `remote_read_file` / `remote_exec`,
...
// tools (model tools) + systemPrompt (per-turn workspace hint) are required.
// webServer is read lazily via ctx.get so it is NOT in inject.
...
export async function apply(ctx, config) {
  ...
  const tools = [
    defineTool({
      ...
      async execute() {
        ...
        const lines = [
          ...
          `Current remote workspace: ${s.workspace || '(none — call remote_pick_workspace to set one)'}`,
        ...
        if (s.host && s.workspace) {
          ...
          lines.push('No host + workspace configured — call remote_connect with a host to get started.')
        }
    ...
    defineTool({
      name: 'remote_connect',
      description:
        'Connect SSH to a remote host for remote workspace work. Provide host (required), user, optional password or privateKeyPath/port. Once connected, call remote_pick_workspace to pick the workspace directory this session should work in.',
      parameters: {
      ...
      async execute(args) {
        ...
        if (!host) throw new Error('remote_connect: host is required')
        pool.setTarget({
          host,
    ...
    defineTool({
      name: 'remote_pick_workspace',
      description:
        'Set the remote workspace directory this session should treat as its working root on the connected remote. Verifies it exists (a directory). Use remote_list_dir to browse first if unsure.',
      parameters: {
      ...
      async execute(args) {
        ...
        if (!p || p === '/') throw new Error('remote_pick_workspace: path must be an absolute directory')
    ...
    defineTool({
      ...
      async execute(args) {
        ...
        if (!p) throw new Error('remote_list_dir: no path and no remote workspace set')
    ...
    defineTool({
      ...
      async execute(args) {
        const p = normalizeRemotePath(String(args.path || ''))
        if (!p) throw new Error('remote_read_file: path is required')
    ...
    defineTool({
      ...
      async execute(args) {
        ...
        if (!cmd) throw new Error('remote_exec: command is required')
  ...
  ctx.systemPrompt.section({
    ...
    text: () => {
      ...
      return (
        ...
        'Use the remote_* tools (remote_list_dir / remote_read_file / remote_exec) to inspect and act on files on the remote host. Treat this directory as the working root for this task.'
      )
    },
  })
      ...
      handler: (invocation) => {
        ...
        return {
          ...
          text:
            ...
            `\nUse tools: remote_list_dir / remote_read_file / remote_exec.` +
