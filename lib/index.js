...
export async function apply(ctx, config) {
   ...
   * changes the fingerprint, so the next /current re-applies it. */
  ...
  const setCurrent = async (id) => {
    ...
    lastAppliedMachine = fingerprint
    return true
  }
  ...
  const tools = [
    ...
    defineTool({
      ...
      async execute(args) {
        ...
        const effUser = eff.username || user
        const effPort = Number(eff.port) || port || 22
        if (args.save !== false) {
          ...
          if (i >= 0) {
            machines[i] = { ...machines[i], ...rec, useSshConfig, id: machines[i].id, password: rec.password || machines[i].password || '' }
          ...
          pool.setTarget({
            host: eff.host, port: effPort, username: effUser,
        ...
        try {
          ...
          if (res.code !== 0 && !res.stdout) {
            audit('connect', `connect ${effUser}@${eff.host}:${effPort}`, res.code)
            return { text: 'connect failed: ' + (res.stderr || 'exit ' + res.code) }
          }
          audit('connect', `connect ${effUser}@${eff.host}:${effPort}`, 0)
          const via = useSshConfig ? `\n(ssh-config alias "${host}" → ${effUser}@${eff.host}:${effPort}` + (eff.sshConfigWarnings && eff.sshConfigWarnings.length ? `; ${eff.sshConfigWarnings.join(' ')}` : '') + ')' : ''
          return { text: `Connected to ${eff.host} as ${config.username}.${via}\n\npick a workspace with rw_pick_workspace (path=<abs>).` }
        } catch (err) {
          throw new Error(friendlyMessage(err, { host: eff.host, port: effPort }))
        }
      },
    }),
