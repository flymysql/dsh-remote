...
import { resolveMirror, poolKey } from './binding.js'
...
export async function apply(ctx, config) {
  ...
  const machineRecordFor = ({ host, port, username }) =>
    machines.find((m) => m.host === host && Number(m.port) === Number(port) && (m.username || '') === (username || '')) || null
  ...
  const poolForMachine = (target) => {
    ...
    const created = new SshPool(poolConfig)
  ...
  const listDirStructured = async (p, sshPool = pool) => {
    const target = normalizeRemotePath(p || '/')
    let sftp
    try {
      sftp = await sshPool.sftp()
    } catch (err) {
   ...
   * @returns {{pool: SshPool, ws: string, host: string, username: string, port: number, mirrorDir: string|null, bound: boolean}}
   *   `bound` is false when the caller has no remote workspace, in which case
   *   the active-machine pool and shared workspace are returned so
   *   machine-scoped tools (`rw_connect`, `rw_info`) keep working.
   */
   ...
   * @returns {ReturnType<typeof bindingFor>} a binding whose `ws` is non-empty.
   */
  const requireBinding = (exec, tool) => {
    const b = bindingFor(exec)
    if (!b.ws) {
      throw new Error(
        `${tool}: no remote workspace for this session. `
        + 'Open a session whose workspace is a remote mirror, or run rw_connect + rw_pick_workspace first.',
      )
    }
  ...
  const tools = [
    ...
    defineTool({
      ...
      async execute(args, exec) {
        ...
        // to when it has one; only an unbound (local) session falls back to the
        // active machine, which is the "set up a new remote" flow.
        const b = bindingFor(exec)
    ...
    defineTool({
      ...
      async execute(args, exec) {
        const b = bindingFor(exec)
        if (!b.host) throw new Error('rw_stat: no remote host for this session — run rw_connect first')
        const p = normalizeRemotePath(String(args.path || ''))
    ...
    defineTool({
      ...
      async execute(args, exec) {
        const b = bindingFor(exec)
        if (!b.host) throw new Error('rw_read_file: no remote host for this session — run rw_connect first')
        const p = normalizeRemotePath(String(args.path || ''))
    ...
    defineTool({
      ...
      async execute(args, exec) {
        const b = bindingFor(exec)
        if (!b.host) throw new Error('rw_write_file: no remote host for this session — run rw_connect first')
        const p = normalizeRemotePath(String(args.path || ''))
    ...
    defineTool({
      ...
      async execute(args, exec) {
        const b = bindingFor(exec)
        if (!b.host) throw new Error('rw_edit: no remote host for this session — run rw_connect first')
        const p = normalizeRemotePath(String(args.path || ''))
    ...
    defineTool({
      ...
      async execute(args, exec) {
        const b = bindingFor(exec)
        if (!b.host) throw new Error('rw_append: no remote host for this session — run rw_connect first')
        const p = normalizeRemotePath(String(args.path || ''))
    ...
    defineTool({
      ...
      async execute(args, exec) {
        const b = bindingFor(exec)
        if (!b.host) throw new Error('rw_mkdir: no remote host for this session — run rw_connect first')
        const p = normalizeRemotePath(String(args.path || ''))
    ...
    defineTool({
      ...
      async execute(args, exec) {
        const b = bindingFor(exec)
        if (!b.host) throw new Error('rw_remove: no remote host for this session — run rw_connect first')
        const p = normalizeRemotePath(String(args.path || ''))
    ...
    defineTool({
      ...
      async execute(args, exec) {
        const b = bindingFor(exec)
        if (!b.host) throw new Error('rw_move: no remote host for this session — run rw_connect first')
        const p = normalizeRemotePath(String(args.path || ''))
    ...
    defineTool({
      ...
      async execute(args, exec) {
        const b = bindingFor(exec)
        if (!b.host) throw new Error('rw_download: no remote host for this session — run rw_connect first')
        const p = normalizeRemotePath(String(args.path || ''))
    ...
    defineTool({
      ...
      async execute(args, exec) {
        const b = bindingFor(exec)
        if (!b.host) throw new Error('rw_upload: no remote host for this session — run rw_connect first')
    ...
    },
    {
      ...
      handler: async (req, res) => {
        try {
          await pool.detect()
          const q = new URL(req.url, 'http://localhost').searchParams
          const raw = q.get('path') ? decodeURIComponent(q.get('path')) : wsPath()
          const canon = normalizeRemotePath(raw)
          const win = pool.platform === 'windows'
          ...
          if (canon === '/' || raw === '' || raw === '/') {
            if (win || pool.platform !== 'posix') {
              const drives = await listRemoteDrives()
          ...
          const out = await listDirStructured(canon)
          const items = out.items.map((it) => ({
            ...it,
            // full display-form path so the client never joins paths itself
            path: toDisplayPath(joinRemotePath(canon, it.name), pool.platform),
          }))
          return sendJson(res, 200, { path: toDisplayPath(out.path, pool.platform), platform: win ? 'windows' : 'posix', items })
        } catch (err) {
          return sendJson(res, 500, { error: String((err && err.message) || err) })
        }
      },
