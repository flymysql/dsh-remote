...
export const Config = z.object({
  ...
  /** Jump host / bastion: connect through this machine first. */
  proxy: z.object({
    host: z.string(),
    port: z.number().step(1).min(1).max(65535).default(22),
    username: z.string().default(''),
  ...
  const tools = [
    ...
    defineTool({
      ...
      async execute(args) {
        ...
        const sftp = await pool.sftp()
        if (args.mkdir !== false) await mkdirRemoteDirs(sftp, p)
    ...
    },
    {
      ...
      handler: async (req, res) => {
        ...
        try {
          ...
          if (!st) await mkdirRemoteDirs(sftp, p)
          await sftp.writeFile(p, buf)
      ...
      handler: async (req, res) => {
        ...
        try {
          ...
          if (op === 'write' || op === 'append') {
            ...
            if (op === 'write' && !(await sftp.stat(p).catch(() => null))) await mkdirRemoteDirs(sftp, p)
            await sftp.writeFile(p, buf)
            audit(op, `${op} ${p}`, 0)
