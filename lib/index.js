...
export async function apply(ctx, config) {
  ...
  const tools = [
    ...
    defineTool({
      ...
      async execute(args) {
        ...
        let full = cmd
        if (cwd) {
          if (pool.platform === 'windows' && pool.gitBashPath) {
  ...
  ctx.systemPrompt.section({
    ...
    text: (promptContext) => {
      ...
      return (
        '## Remote workspace\n' +
        `Current remote workspace: ${config.username || 'user'}@${config.host}:${toDisplayPath(remotePath, pool.platform)}\n` +
