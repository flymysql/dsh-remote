...
export async function apply(ctx, config) {
  ...
  const tools = [
    ...
    defineTool({
      ...
      parameters: {
        required: ['path'],
        path: { type: 'string', description: 'Absolute remote directory path, e.g. /home/dev/code/project' },
      },
      output: {
        schema: { type: 'object', additionalProperties: false, required: ['text'], properties: { text: { type: 'string' } } },
        render: (_a, a) => [{ type: 'text', text: a.text }],
    ...
    defineTool({
      ...
      parameters: {
        required: ['command'],
        command: { type: 'string', description: 'Shell command (run on the remote host)' },
      },
      output: {
        schema: { type: 'object', additionalProperties: false, required: ['text'], properties: { text: { type: 'string' } } },
        render: (_a, a) => [{ type: 'text', text: a.text }],
    ...
    defineTool({
      ...
      parameters: {
        ...
        path: { type: 'string', description: 'Absolute remote file path' },
        content: { type: 'string', description: 'File content to write (overwrites existing file)' },
        mkdir: { type: 'boolean', description: 'Create missing parent directories (default true)' },
      },
      output: {
        schema: { type: 'object', additionalProperties: false, required: ['ok'], properties: { ok: { type: 'boolean' }, bytes: { type: 'integer' }, text: { type: 'string' } } },
    ...
    defineTool({
      ...
      parameters: {},
      output: {
        schema: { type: 'object', additionalProperties: false, required: ['ok'], properties: { ok: { type: 'boolean' }, text: { type: 'string' } } },
        render: (_a, a) => [{ type: 'text', text: a.text }],
      },
      async execute() {
        pool.close()
