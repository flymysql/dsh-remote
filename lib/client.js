    ...
    function DirPicker(props) {
      ...
      const loadLevels = (id, p, toIndex) => {
        ...
        .then((res) => {
          ...
          // keep both dirs and files; the picker lists all, enter loads a dir
          const node = { path: real, dirs: parsed.filter((it) => it.dir), all: parsed }
          setLevels((prev) => {
...
// appending that directory as the new deepest level (path is pieced together).
      ...
      function renderCascade() {
        ...
        return React.createElement('div', { style: { display: 'flex', flexDirection: 'column', gap: 6 } },
          React.createElement('div', { style: { display: 'flex', gap: 6, alignItems: 'center', minHeight: 26 } },
          ...
          React.createElement('div', { style: boxS, ...{ height: 240, overflowY: 'auto', overflowX: 'hidden' } },
            loading ? React.createElement('div', { style: { opacity: 0.7, padding: 6 } }, '加载中…')
              : (entries.length ? entries.slice(0, 400).map((it, i) => React.createElement('div', {
                  ...
                  style: { padding: '3px 6px', borderRadius: 4, cursor: it.dir ? 'pointer' : 'default', color: it.dir ? T.ok : T.label, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', display: 'flex', gap: 6, alignItems: 'center', fontSize: 13 },
                },
                  React.createElement('span', { style: icons }, it.dir ? '📁' : '📄'),
                  React.createElement('span', { style: { overflow: 'hidden', textOverflow: 'ellipsis' } }, it.name),
                )) : React.createElement('div', { style: { opacity: 0.6, padding: 6 } }, '（空目录）')),
          ),
        )
      }
