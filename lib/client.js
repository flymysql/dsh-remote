...
// appending that directory as the new deepest level (path is pieced together).
    ...
    //
    // We register two things through the public `ctx.betterSidebar` service
    // (v0.4.0+; gracefully skipped when dsh-better-sidebar is not installed):
    //   • a tab `dsh-remote:explorer` — a live remote file tree (dirs expand
    //     over /dsh-remote/ls; files open in the remote viewer below),
    //   • a file viewer `dsh-remote:remote` (fetchStrategy 'custom') — renders
    //     any path that looks remote (inside the current remote workspace, or
    //     prefixed with the machine tag) by reading it over /dsh-remote/read.
    //
    ...
    function RemoteExplorerTab(props) {
      ...
      const refreshStatus = () => {
        fetchRemoteStatus().then((s) => {
          setStatus(s)
          if (s && s.workspace) setLevels((prev) => (prev.length ? prev : [{ path: s.workspace, items: null }]))
        })
      }
      ...
      const jumpTo = (p) => {
        const idx = levels.findIndex((lv) => lv.path === p)
        if (idx < 0) return
    ...
    // ── Remote file viewer (registered through betterSidebar) ─────────────
    // fetchStrategy 'custom': the host calls load(path, scope, signal) and the
    // returned value rides as customData into the component. We read the file
    // live over /dsh-remote/read and render text (or a binary notice).
    const remoteViewerLoad = async (path, scope, signal) => {
      const data = await readRemoteFile(path, 256 * 1024)
      return data
    }

    function RemoteFileViewer(props) {
      const { path, customData } = props
      const data = (customData && typeof customData === 'object') ? customData : null
      if (!data) return React.createElement('div', { style: { padding: 12, fontSize: 13, opacity: 0.8 } }, '加载中…')
      if (data.binary) {
        return React.createElement('div', { style: { padding: 12, fontSize: 13, color: '#e6c07b' } },
          '二进制文件 (' + (data.size != null ? data.size + ' bytes' : '未知大小') + ') — 远程查看器为只读，请用 rw_download / 本地镜像查看。')
      }
      return React.createElement('div', { style: { height: '100%', overflow: 'auto', fontSize: 13, lineHeight: 1.55, fontFamily: 'ui-monospace, SFMono-Regular, Menlo, Consolas, monospace', padding: '10px 12px', whiteSpace: 'pre-wrap', wordBreak: 'break-word' } },
        data.content != null ? data.content : '（空文件）',
        data.truncated ? React.createElement('div', { style: { color: '#e6c07b', fontSize: 12, marginTop: 8 } }, '… 内容已截断（只读远程查看，完整文件请用 rw_read_file / 下载）') : null,
      )
    }
    ...
    function registerSidebarIntegration(ctx) {
      ...
      if (!bs || typeof bs.registerTab !== 'function' || typeof bs.registerFileViewer !== 'function') return
      const disposers = []
      try {
        ...
        disposers.push(bs.registerFileViewer({
          id: SIDEBAR_VIEWER_ID,
          title: () => '远程文件 (dsh-remote)',
          exts: [],
          priority: 200, // higher than builtins (0 / -100) so remote paths win
          fetchStrategy: 'custom',
          load: remoteViewerLoad,
          component: (props) => React.createElement(RemoteFileViewer, props),
        }))
      } catch (e) {
    ...
    function apply(ctx) {
      ...
      // better-sidebar integration: register the remote explorer tab + viewer
      // when dsh-better-sidebar is installed. Guarded — the plugin is optional
      // and everything above works without it. ctx.effect makes the disposer
      // run on fiber disposal (HMR-safe), mirroring the service's contract.
      const bsDispose = registerSidebarIntegration(ctx)
      if (bsDispose) {
        ctx.effect(() => bsDispose, 'dsh-remote.betterSidebar')
      }
    }
