  ...
  factory: (require) => {
    ...
    function parseLs(text) {
      ...
      for (const ln of String(text || '').split('\n')) {
        const m = ln.match(/^(d[-r][rwx-]{9})\s+[^ ]+\s+[^ ]+\s+[^ ]+\s+[^ ]+\s+\d{4}-\d{2}-\d{2}([T ]+\S+)?\s+(.+)$/)
        if (!m) continue
      ...
      return React.createElement('div', { style: { padding: 14, minWidth: 460, maxWidth: 720 } },
        ...
        tab === 'local'
          ? React.createElement('div', { style: { display: 'flex', flexDirection: 'column', gap: 8 } },
              React.createElement('div', { style: { fontSize: 12, opacity: 0.8 } }, '点击按钮打开系统文件夹选择器，选中本地目录即作为本地工作区。'),
              React.createElement('button', { style: buttonS, onClick: chooseLocal, disabled: loading }, '打开系统文件夹选择器'),
            )
          : React.createElement('div', { style: { display: 'flex', flexDirection: 'column', gap: 8 } },
