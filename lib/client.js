...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function DirPicker(props) {
      ...
      //  - postedCurrentRef: POST /current only when the machine CHANGES
      ...
      // full reconnect. Never rejects: a failed switch is retried next time.
      const ensureCurrent = (id) => {
        if (!id || postedCurrentRef.current === id) return Promise.resolve()
        postedCurrentRef.current = id
        return api('POST', '/dsh-remote/current', { id }).catch(() => {
          if (postedCurrentRef.current === id) postedCurrentRef.current = ''
        })
      }
      ...
      // `fresh` bypasses the cache (browse navigation / post-mkdir refresh).
      ...
      const fetchDirList = (id, dirParam, fresh) => {
        ...
        .then((res) => {
          ...
          lsCacheRef.current.set(key, { at: Date.now(), node })
          return node
        })
      ...
      React.useEffect(() => { if (open) {
        ...
        // listings cached during a previous open (TTL bounds staleness within
        // one open; reopening should see the live tree).
      ...
      const loadSuggestions = (raw, mid) => {
        ...
        // Only the newest keystroke owns the dropdown: a slow older /ls must
        // never overwrite the suggestions for the text now in the input.
        const seq = ++suggestSeqRef.current
        fetchDirList(id, parent || '/').then((node) => {
          if (seq !== suggestSeqRef.current) return
          const matches = node.dirs
      ...
      const continueSuggest = (dir) => {
        ...
        fetchDirList(machineId, target).then((node) => {
          if (seq !== suggestSeqRef.current) return
          const kids = node.all.slice(0, 40).map((it) => it.path)
          setSuggest(kids)
