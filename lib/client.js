  ...
  factory: (require) => {
    ...
    function DirPicker(props) {
      ...
      const loadLevels = (id, p, toIndex) => {
        ...
        .then((res) => {
          ...
          const list = Array.isArray(res && res.items)
            ? res.items.map((it) => ({ name: it.name, dir: it.type === 'dir', size: it.size, mtime: it.mtime, path: it.path, drive: !!it.drive }))
      ...
      const loadSuggestions = (raw, mid) => {
        ...
        .then((res) => {
          const list = Array.isArray(res && res.items)
            ? res.items.map((it) => ({ name: it.name, dir: it.type === 'dir', path: it.path }))
            : []
          const matches = list
