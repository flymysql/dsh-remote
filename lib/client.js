...
// appending that directory as the new deepest level (path is pieced together).
      ...
      function renderDirPopup() {
        ...
        return React.createElement('div', {
          ...
          React.createElement('div', { style: { background: T.bg2, border: '1px solid ' + T.borderStrong, borderRadius: 10, boxShadow: '0 10px 40px rgba(0,0,0,0.3)', width: 'min(600px, 92vw)', display: 'flex', flexDirection: 'column', maxHeight: 'min(420px, 80vh)', overflow: 'hidden' }, onClick: (e) => e.stopPropagation() },
            ...
            React.createElement('div', { style: { display: 'flex', gap: 8, justifyContent: 'flex-end', alignItems: 'center', padding: '8px 12px', borderTop: '1px solid ' + T.border } },
              ...
              React.createElement('button', { style: { ...buttonS, fontWeight: 600 }, onClick: pickRemoteLevel, disabled: busy || loading }, busy ? '镜像中…' : '设为远程工作区'),
            ),
          ),
        )
      }
        ...
        tab === 'local'
          ...
          : React.createElement('div', { style: { display: 'flex', flexDirection: 'column', gap: 8 } },
              React.createElement('div', { style: { display: 'flex', gap: 6, alignItems: 'center' } },
                ...
                React.createElement('select', { value: machineId, onChange: (e) => { const id = e.target.value; setMachineId(id); setLevels(null); if (id) loadLevels(id, '', 0) }, style: { ...inputS, maxWidth: '100%', minWidth: 160, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' } },
              ...
              // 一行触发按钮（占位扁平，不撑开布局）：点开悬浮目录选择弹层
              React.createElement('button', {
                onClick: () => { if (machineId) setPopOpen(true) },
                disabled: !machineId,
                style: { ...inputS, cursor: 'pointer', textAlign: 'left', display: 'flex', alignItems: 'center', gap: 6 },
              },
                React.createElement('span', null, '🔽'),
                React.createElement('span', { style: { overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', flex: 1 } }, levels && levels.length ? levels[levels.length - 1].path : (machineId ? '点击选择远程目录' : '先选远程机器')),
              ),
              err ? React.createElement('div', { style: { color: T.danger, fontSize: 12 } }, err) : null,
              React.createElement('div', { style: { display: 'flex', gap: 8, justifyContent: 'flex-end', alignItems: 'center' } },
