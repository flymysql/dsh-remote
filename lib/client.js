...
// appending that directory as the new deepest level (path is pieced together).
    ...
    function registerSidebarIntegration(ctx) {
      ...
      // tree the built-in Files tab lists). A path-bearing openTab expands
      // the panel and lands the tab in sight; on the single:true explorer
      // descriptor the same type focuses the existing tab.
      fetchRemoteStatus().then((s) => {
