  ...
  factory: (require) => {
    ...
    function parseLs(text) {
      ...
      for (const ln of String(text || '').split('\n')) {
        ...
        const mode = parts[0]
        // ls -l columns: mode links owner group size month day [year|hh:mm] name...
        // The name is everything after the fixed leading columns. Because the date
        // field shape varies (year vs time), swallow the token before the name too.
    ...
    function DirPicker(props) {
      ...
      const jumpTo = (segPath) => loadRemote(machineId, segPath || '/')
      const pickRemote = () => {
        ...
        tab === 'local'
          ...
          : React.createElement('div', { style: { display: 'flex', flexDirection: 'column', gap: 8 } },
              React.createElement('div', { style: { display: 'flex', gap: 6, alignItems: 'center' } },
                ...
                React.createElement('select', { value: machineId, onChange: (e) => useMachine(e.target.value), style: { ...inputS, maxWidth: '100%', minWidth: 120, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' } },
              ...
              React.createElement('div', { style: { display: 'flex', gap: 6, alignItems: 'center', minHeight: 28 } },
                React.createElement('button', { style: { ...buttonS, padding: '3px 10px' }, onClick: goUp, disabled: !machineId || loading || path === '/' || !String(path).replace(/\/+$/, '') }, '↑ 上一级'),
                React.createElement('div', { style: { fontSize: 11, opacity: 0.75, fontFamily: 'monospace', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' } },
                  breadcrumb(machineId, path, jumpTo)),
              ),
              err ? React.createElement('div', { style: { color: T.danger, fontSize: 12 } }, err) : null,
              React.createElement('div', { style: boxS, ...{ height: 240, overflowY: 'auto', overflowX: 'hidden' } },
                loading ? React.createElement('div', { style: { opacity: 0.7, padding: 6 } }, '加载中…')
                  : (items && items.length ? items.slice(0, 400).map((it, i) => React.createElement('div', { key: i, title: it.name, onClick: () => (it.dir ? enter(it.name) : null), style: { padding: '3px 6px', borderRadius: 4, cursor: it.dir ? 'pointer' : 'default', color: it.dir ? T.ok : T.label, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', display: 'flex', gap: 6 } },
                        React.createElement('span', null, it.dir ? '📁' : '📄'),
                        React.createElement('span', { style: { overflow: 'hidden', textOverflow: 'ellipsis' } }, it.name))) : React.createElement('div', { style: { opacity: 0.6, padding: 6 } }, '先选机器，再点目录进入下一级，或在输入框填路径')),
              ),
