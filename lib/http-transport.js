...
export function connectionRoute(route) {
  if (route.kind !== 'exact' || !route.path.startsWith('/dsh-remote/')) {
    throw new Error('dsh-remote: expected an exact plugin route')
  }
  return {
    path: '/api' + route.path,
    methods: ['GET', 'POST'],
...
export function registerHttpTransports(ctx, routes) {
  ...
  ctx.inject(['connection'], (inner) => {
    ...
    const disposers = routes.map((route) => connection.fetch.register(connectionRoute(route)))
